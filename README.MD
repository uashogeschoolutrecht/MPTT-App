...
This app connects to a Movella/Xsens DOT sensor via BLE and displays a tilt ball UI.
...
```sh
python -m movella_dot_py.app.spiral_tracking
```
...
- Target path now uses an 8-lobed rose curve r(θ) = R0 + A·cos(8θ), scaled to the axis range with R_min > 0 so it never enters the center.
